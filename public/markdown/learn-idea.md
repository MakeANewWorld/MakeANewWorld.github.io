...
然後點兩下`runClient`(執行客戶端)，也就是跑(執行)Minecraft，你會發現IDEA右上角多出幾個東西。  
![process_control](images/learn-idea/process_control.png)

**綠色**的是`重新執行(執行)`一次Minecraft。  
...
然後你點一下那個正在`輸出`的區域，我們稱做**控制台**，裡面一樣可以選中、複製，但通常是**不能**貼上、輸入的，除非電腦問你事情(`input`)。  
按下`Ctrl+F`，會出現搜尋框，在裡面輸入:`Hello Fabric world!`，就可以快速找到那行輸出。  
![find_log](images/learn-idea/find_log.png)

然後你就會發現一件神奇的事!編輯器裡面這一行被引號包起來的文字**和電腦說的話對應上**了!  
也許可以嘗試**改改看**內容...?  
