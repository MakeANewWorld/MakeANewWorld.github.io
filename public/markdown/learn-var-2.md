...
float health = 20.0;
```
會不能執行，因為電腦**預設小數為double**，使用float可能只是為了**節省記憶體**(因為float類型能儲存資料的大小上限比double小)。  
通常我們的**習慣**還是會在double後面加上`D`，像下面這樣:  
```java
double a = 1.0D;
```
也許聰明的你已經發現了，我們還有一個東西:`名字`，我們要在電腦裡面**儲存資料**的時候會用`類型 名字 = 值;`的形式去儲存。  
名字的不同對電腦來說**沒有意義**(但不能是某些關鍵字，如果你有試到就會拋錯誤)，只是讓你認得**資料**在哪個名字的地方儲存 (`取出` or `放入`)。  
所以我們也可以這樣寫(注意只能改被 {} 包住的部分喔):  
```java
...
(例如:血量為a、飢餓值為b、道具欄數為c...、行走的速度為z)  
(那修改程式的時候，看那一堆a~z會分不清哪個是什麼)  
...
return health * turns;
```
從上面就知道，電腦中的`加法符號`就是`+`，`減法符號`就是`-`。  
`乘法符號`變成`*`，`除法符號`變成`/`。  
重新載入，測試一下:  
![test_change_code](images/learn-var-2/test_change_code.png)  
成功了! 這時候我又想說，如果他是`400`度，血量就要乘以`1.~`，那會**浪費40度**耶，我想把**剩下的角度除以十**然後直接**加上去**血量。  
...
```
照理來說計算的順序會是從左到右，可是**太不清楚**了吧，萬一我看成`headYaw % 36.0F`呢?  
這種時候我們可以用**括號**:  
```java
float headYaw = entity.getHeadYaw();
float leftoverYawDivide10 = (headYaw % 360.0F) / 10;
...
```
等於不是說他是相同的，是設定等號左邊的資料槽位的資料為右邊那串。  
為何會有兩個`health`呢?  
我幫你翻譯成中文你就看得懂了:  
設定health 為 (health的值 乘上 (頭的角度 除以 360))  
說白了就是先取出來操作再放進去。  
所以我們也可以得出一個結論，在Java當中，我們也可以對於這種算式做簡化: 
```java
a = a + b;
```
也就是
...
health++;
```
不過要注意順序在這裡雖然沒有差異，但其實如果是`health++`會先給出`health`欄位資料的值，然後對`health`欄位的資料的值+1。  
如果是`++health`，會先對`health`欄位的資料的值+1，然後再給出`health`欄位資料的值。  
我們來看一段範例:  
```java
float a = 1.0F;
float b = a++;
```
a變成2，b還是1，因為先給出了a，然後設定a為a+1。  
```java
float a = 1.0F;
float b = ++a;
```
a變成2，b變成2，因為先設定a為a+1，然後才給出了a。  
最後我們的程式碼會長成這樣:  
```java
float headYaw = entity.getHeadYaw();
float leftoverYawDivide10 = (headYaw % 360.0F) / 10;
